...
public class Cli {
  ...
  public Cli parse(final String... args) {
    initCommander();
  ...
  public void initCommander() {
    ...
    commander = JCommander.newBuilder().programName("bodkin").addCommand("build", build)
        .addCommand("serve", serve).addCommand("clean", clean).build();
  }
}
